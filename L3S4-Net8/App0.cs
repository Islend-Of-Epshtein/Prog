...
public class App0 : INotifyPropertyChanged
{
    ...
    {
        _timer = new DispatcherTimer { Interval = TimeSpan.FromMilliseconds(updateTime) };
    ...
    private static string GetDisplayLanguageName(InputLanguage language) => language.LayoutName switch
    {
    ...
    private void InitLogInFrame() => _ = new LogInFrame(this);
    ...
    public string? GetMethodNameByIndex(int index) =>
        index >= 0 && index < elements.Count ? elements[index].MethodName : null;

    public string? GetNameByIndex(int index) =>
        index >= 0 && index < elements.Count ? elements[index].Name : null;

    public int GetTreeNumberByIndex(int index) =>
        index >= 0 && index < elements.Count ? elements[index].TreeNumber : -1;
    ...
    public void InvokeMethod(string methodName)
    {
        ...
        var mi = GetType().GetMethod(methodName, BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic, null, EmptyTypes, null);
        if (mi != null)
        {
            try
            {
